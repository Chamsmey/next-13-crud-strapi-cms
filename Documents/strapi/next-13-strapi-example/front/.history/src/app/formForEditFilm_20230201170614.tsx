...
interface FormUpdateProps{
    data:any,
    id:any,
    handlerUpdateStatus:(arg:any) => void)
}
const FormEditFilmCom = ({data,id,isUpdated,handlerUpdateStatus}:{data:any,id:any,isUpdated:any}) => {
  ...
  return (
    <section>
      ...
      <section
        id="form-create"
        className={`fixed bg-black/50 w-full h-[100vh] z-[2000] top-0 left-0 ${
        ...
        >
          ...
          <div className="mt-4 row-span-1 grid grid-cols-2 gap-1">
            ...
            className="block px-1 py-2 rounded-sm w-full flex-1 outline outline-1 outline-offset-0 outline-gray-300 sm:text-sm focus:outline-indigo-500 bg-white/10 backdrop-blur-sm"
              ...
              name="title"
              type="text"
              value={`${isUpdated? data.tile : film.title}`}
            ...
            <input
              className="block px-1 py-2 rounded-sm w-full flex-1 outline outline-1 outline-offset-0 outline-gray-300 sm:text-sm focus:outline-indigo-500 bg-white/10 backdrop-blur-sm"
              id="director"
              value={`${isUpdated? data.director : film.director}`}
              name="director"
              type="text"
              onChange={handleInputs}
          ...
          <div className="mt-4 row-span-1 ">
            <input
              ...
              id="released"
              name="released"
              value={`${isUpdated? data.released : film.released}`}
