...
const FormEditFilmCom = ({dat,isUpdated,handlerUpdateStatus}:FormUpdateProps) => {
  ...
  useEffect(()=>{
    if(data){
        ...
        setFilm({
          ...
          <poster:data className=""></poster:data>
        })
    }
    else{
        setIsUpdate(false);
  ...
  const handleSumit = async (e: any) => {
    ...
    if (image) {
      axios({
        url: `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/films`,
        ...
        >
          ...
          <div className="mt-4 row-span-1 grid grid-cols-2 gap-1">
            ...
            className="block px-1 py-2 rounded-sm w-full flex-1 outline outline-1 outline-offset-0 outline-gray-300 sm:text-sm focus:outline-indigo-500 bg-white/10 backdrop-blur-sm"
              ...
              name="title"
              type="text"
              value={`${isUpdated? data.attributes.title : film.title}`}
            ...
            <input
              className="block px-1 py-2 rounded-sm w-full flex-1 outline outline-1 outline-offset-0 outline-gray-300 sm:text-sm focus:outline-indigo-500 bg-white/10 backdrop-blur-sm"
              id="director"
              value={`${isUpdated? data.attributes.director : film.director}`}
              name="director"
              type="text"
              onChange={handleInputs}
          ...
          <div className="mt-4 row-span-1 ">
            <input
              ...
              id="released"
              name="released"
              value={`${isUpdated? data.attributes.released : film.released}`}
