...
const NavCom = () => {
  ...
  const [acronymName, setAcronymName] = useS
  // let fake:string = 'cham smey';
  var acronym = user?.match(/\b(\w)/g).join('');
  console.log('user name split', acronym);
    ...
    <header className="lg:max-w-[1400px] md:max-w-[1200px] w-full  mx-auto bg-black/50 backdrop-blur-0 fixed top-0  z-10">
      <nav className="flex justify-between py-2 px-1">
        ...
        <div className="px-2 flex text-white items-center">
          ...
          {user && <h1 className="p-2 bg-blue-500 rounded-full">{user}</h1>}
        </div>
      </nav>
    </header>
  );
