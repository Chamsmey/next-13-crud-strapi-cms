  ...
  const handleSumit = async (e: any) => {
    e.preventDefault();
      ...
      axios({
        ...
        .then((res) => {
          console.log("🚀 ~ file: formCreatefilm.tsx:53 ~ .then ",  res.data.data.id)
        ...
        .then((refId) => {
          ...
          axios({
           ...
           url: `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/upload/files/${data?.attributes?.poster?.data?.id}`,
            method: "DELETE", url: `${process.env.NEXT_PUBLIC_STRAPI_URL}/api/upload/}`,
            method: "POST",
            data: formData,
            headers: {
